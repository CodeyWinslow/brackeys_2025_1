...
var results_per_word : Array[RecordedLetterResult]
...
func _consume_letter(letter : LetterController, result : LetterResultType):
	...
	recorded_results.append(letter_result)
...
func _spawn_next_letter(time_spent : float):
	...
	if instance:
		...
		instance.set_info(current_letter, null)
...
func _update_text_preview():
	...
	var word_index = 0
	var letter_index = 0
	var i = len(recorded_results)
	while i > 0:
		if i >= len(song_words[word_index]):
			i -= len(song_words[word_index])
			word_index += 1
		else:
			letter_index = i
			i = 0
	...
	for res in recorded_results:
		match res.result:
			LetterResultType.Hit:
				output += "[color=green]%s[/color]" % res.letter
			_:
				output += "[color=red]%s[/color]" % res.letter
	
	if not recorded_results.is_empty() and letter_index == 0:
		output += " "
	
	if word_index < len(song_words):
		output += song_words[word_index].substr(letter_index)
		
	if word_index + 1 < len(song_words):
		output += ' ' + ' '.join(song_words.slice(word_index+1, len(song_words)))
	
	#output += ' '.join(song_words.slice(0, spawn_word_index)) + '[/color]'
	#if spawn_word_index < len(song_words) and spawn_word_index > 0:
		#output += ' '
	#output += ' '.join(song_words.slice(spawn_word_index, len(song_words)))
